...
@dc.dataclass()
class Plotter:
    def cdf_active_switches (alterations: np.ndarray, routings: np.ndarray, ax: plt.axes = None) -> None:
        ...
        x = np.linspace(min_indx, max(active_percentage))
        y = ecdf(x)

        return ax.step(x, y)

    def cdf_active_ports (alterations: np.ndarray, routings: np.ndarray, ax: plt.axes = None) -> None:
    ...
    def cdf_switches_active_ports (alterations: np.ndarray, routings: np.ndarray, ax: plt.axes = None) -> None:
