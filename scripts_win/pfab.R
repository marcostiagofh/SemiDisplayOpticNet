...
throughput.table <- read.csv(".\\csv_data\\pfabDS-trace_0_5\\throughput.csv")
...
throughput.plot <- throughput.plot + theme(panel.grid.minor = element_blank(),
  ...
  scale_x_continuous(labels = function(x){paste0(x/1000000)}, limits = c(0, 3000000), breaks = seq(0, 4000000, 1000000))

plot(throughput.plot)

ggsave(filename = "./output/pfabDS-trace/throughput.pdf", units = "cm",
