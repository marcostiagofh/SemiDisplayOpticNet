...
throughput.table <- read.csv(".\\csv_data\\tor\\throughput.csv")
...
throughput.plot <- throughput.plot + theme(panel.grid.minor = element_blank(),
  ...
  scale_x_continuous(labels = function(x){paste0(x/10000)}, limits = c(0, 25000))

plot(throughput.plot)

ggsave(filename = "./output/tor/throughput.pdf", units = "cm",
